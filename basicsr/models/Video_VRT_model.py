...
class ModelVRT(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # fix the flow network
        self.fix_keys = ['spynet']
        for name, param in self.net_g.named_parameters():
            if any([key in name for key in self.fix_keys]):
                param.requires_grad_(False)
        self.net_g.spynet.training = False
    ...
    def setup_optimizers(self):
        ...
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                if "deform" in k:
                    optim_params_deform.append(v)
                    logger.warning(f"deformoffset lr {k}")
                else:
        ...
        deform_ratio = train_opt['deform_ratio']
        optim_type = train_opt['optim_g'].pop('type')
        if optim_type == 'Adam':
            self.optimizer_g = torch.optim.AdamW([{'params': optim_params}, {'params': optim_params_deform,
